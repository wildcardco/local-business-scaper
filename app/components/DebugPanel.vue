    ...
    >
      ...
      <div class="flex items-center justify-between px-4 py-2 bg-elevated border-b border-default">
        <div class="flex items-center gap-2">
          ...
          <span class="font-semibold text-highlighted">API Debug Console</span>
      ...
      <div v-if="!isMinimized" class="h-[calc(100%-48px)] overflow-y-auto p-2 space-y-1">
        <div v-if="logs.length === 0" class="text-center py-8 text-dimmed">
        ...
        <div
          v-for="log in logs"
          :key="log.id"
          class="flex items-start gap-2 px-2 py-1.5 rounded hover:bg-elevated/50 group"
        >
          <span class="text-dimmed text-xs shrink-0 mt-0.5">
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2">
              ...
              <span v-if="log.duration" class="text-xs text-dimmed">
                {{ log.duration }}ms
              </span>
            </div>
            <p class="text-toned text-xs mt-0.5 truncate" :title="log.url">
              {{ log.message || log.url }}
            </p>
          </div>
        </div>
